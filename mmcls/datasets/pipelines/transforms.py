...
@PIPELINES.register_module()
class RandomResizedCrop_lanes(object):
    """Crop the given image to random size and aspect ratio.
    ...
    def get_params(img, scale, ratio, max_attempts=10):
        ...
        height = img.shape[0]
        width = img.shape[1]
        ratio_lanes = [0.75, 0.9]

        for _ in range(max_attempts):
            target_width = int(round(random.uniform(*ratio_lanes) * width))
            target_height = int(round(random.uniform(*ratio_lanes) * height))
