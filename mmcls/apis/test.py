...
def single_gpu_test_get_diff_img(model,
                    data_loader,
                    show=False,
                    out_dir=None,
                    **show_kwargs):
    ...
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            res = list()
            path = 'bad_case'
            for i, (pred_cla, img, img_meta) in enumerate(zip(pred_class, imgs, img_metas)):
                target_class = img_meta['ori_filename'].split('/')[0]
                img_name = img_meta['filename']
                if pred_cla != target_class:
                    img_path = osp.join(path, target_class)
                    pre_name = pred_cla + str(i) + '.jpg'
                    name = osp.join(img_path, pre_name)
                    # img_path = img_path + '/'
                    shutil.copy(img_name, name)
                    # h, w, _ = img_meta['img_shape']
                    # img_show = img[:h, :w, :]
